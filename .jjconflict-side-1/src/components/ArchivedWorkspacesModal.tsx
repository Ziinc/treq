...
export const ArchivedWorkspacesModal: React.FC<
	ArchivedWorkspacesModalProps
> = ({ open, onOpenChange, repoPath }) => {
	const queryClient = useQueryClient();
	const { addToast } = useToast();
	const [selectedId, setSelectedId] = useState<number | null>(null);
	const { data: archivedWorkspaces = [], isPending } = useQuery({
		queryKey: ["archived-workspaces", repoPath],
		queryFn: () => getArchivedWorkspaces(repoPath),
		enabled: open && !!repoPath,
	});
	useEffect(() => {
		if (!archivedWorkspaces.some((workspace) => workspace.id === selectedId))
			setSelectedId(archivedWorkspaces[0]?.id ?? null);
	}, [archivedWorkspaces, selectedId]);
	const selected =
		archivedWorkspaces.find((workspace) => workspace.id === selectedId) ?? null;
	const { data: commits = [], isPending: commitsPending } = useQuery({
		queryKey: ["archived-workspace-commits", repoPath, selectedId],
		queryFn: () => getArchivedWorkspaceCommits(repoPath, selectedId!),
		enabled: open && selectedId !== null,
	});
	const restoreMutation = useMutation({
		mutationFn: (workspaceId: number) =>
			restoreWorkspace(repoPath, workspaceId),
		onSuccess: (workspace) => {
			...
			addToast({
				...
				description: `${workspace.branch_name} is back in your sidebar`,
				type: "success",
			});
		},
		onError: (error) =>
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[900px]">
				<DialogHeader>
					...
					<DialogDescription>
						Select a workspace to review its details and restore it.
					</DialogDescription>
				</DialogHeader>
				<div className="grid min-h-[420px] grid-cols-[280px_minmax(0,1fr)] overflow-hidden rounded-md border">
					<div className="overflow-y-auto border-r bg-muted/20 p-2">
						...
						{!isPending && !archivedWorkspaces.length && (
						...
						{archivedWorkspaces.map((workspace) => (
							<button
								...
								className={`w-full rounded-md px-3 py-2 text-left ${workspace.id === selectedId ? "bg-muted" : "hover:bg-muted/50"}`}
							>
								...
								<div className="mt-1 truncate text-xs text-muted-foreground">
									Updated{" "}
									{formatTime(workspace.refreshed_at ?? workspace.created_at)}
								</div>
							</button>
						))}
					</div>
							...
							<div className="space-y-6">
								<div>
									<h3 className="text-lg font-semibold">
										{selected.title || selected.branch_name}
									</h3>
								...
								<div>
									...
									<div className="mt-1 flex items-center gap-2 font-mono text-sm">
										...
										{selected.target_branch || "Default branch"}
									</div>
								</div>
								<div>
									...
									{commitsPending ? (
										...
										<ul className="divide-y rounded-md border">
											{commits.map((commit) => (
												<li
													key={commit.commit_id ?? commit.short_id}
