...
describe("default agent configuration", () => {
	...
	it("Start an agent session command palette entry uses the configured default_agent, not hardcoded claude", async () => {
		await setSetting("default_agent", "codex");
		...
		const newAgentTerminal = await screen.findByText("Start an agent session");
