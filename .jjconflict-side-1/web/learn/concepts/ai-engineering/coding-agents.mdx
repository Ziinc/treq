...
The underlying model may be the same one used in a chat tool. The difference is that the agent can take actions and keep working across several steps. Chat tools and agents sit at different points on the [AI-assisted software engineering](/learn/concepts/ai-engineering/ai-assisted-software-engineering) spectrum.
...
Always review the complete diff, which is the heart of a working [human-in-the-loop review system](/learn/ai-code-review). Tests may pass because the agent made the same incorrect assumption in both the code and the tests. Check for unrequested changes, new dependencies, missed edge cases, security-sensitive behaviour, and code that departs from established project patterns.
...
[Parallel agents](/learn/parallel-coding-agents) need separate workspaces. If several agents edit the same directory, their changes can conflict or overwrite each other. Give each agent its own worktree, branch, sandbox, or clone, then merge changes only after review.
