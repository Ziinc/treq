...
Implementation plans are structured markdown documents that outline design decisions, step-by-step tasks, and code examples. They're typically generated by AI assistants but can be written manually. Treq parses plans, displays them alongside your terminal, and lets you create workspaces directly from them.
...
Actions include **Execute** (create a workspace from the plan), **Save** (add to history), **Copy** (clipboard), and **Clear** (remove from view).

## Executing a Plan

When you execute a plan, Treq creates a new workspace, sets the plan title as metadata, opens a terminal session, and displays the plan in the panel. Click **Execute Plan**, review or adjust the pre-filled branch name and base branch in the dialog, then click **Create**. You're ready to implement.
...
Plans are stored in `.treq/plans/` as markdown files with accompanying JSON metadata (title, creation date, associated workspace, status).
...
- [Merging Workspaces](merging-workspaces) — Merge completed work
