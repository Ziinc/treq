...
export const SessionTerminal = memo<SessionTerminalProps>(
  function SessionTerminal({
    ...
    const consolidatedTerminalRef = useRef<ConsolidatedTerminalHandle | null>(
      null
    );
      ...
      debouncedParserRef.current(terminalOutput, async (sections) => {
        ...
        const sectionsWithEdits = await Promise.all(
          sections.map(async (section) => {
            try {
              const savedPlan = await loadPlanFromRepo(
                effectiveRepoPath,
                section.id,
                ptySessionId
              );
              if (savedPlan) {
                const hasExplicitEdit =
                  savedPlan.editedAt &&
                  new Date(savedPlan.editedAt) > section.timestamp;
                if (hasExplicitEdit) {
                  return {
                    ...section,
      ...
      ptyWrite(ptySessionId, next)
        ...
        .catch((error) => {
          ...
          addToast({
            title: "Prompt Error",
            description:
              "Unable to deliver the automated instructions to Claude.",
            type: "error",
          });
        });
        ...
        try {
          ...
          if (files.length > 0) {
            ...
            if (filePath) {
              ...
              const escapedPath = filePath.includes(" ")
                ? `"${filePath}"`
                : filePath;
              ...
              if (item.type.startsWith("image/")) {
                ...
                if (blob) {
                  addToast({
                    title: "Image dropped",
                    description:
                      "Image files cannot be pasted directly. Please save and drag the file instead.",
                    type: "info",
                  });
                }
                return;
          ...
          addToast({
            ...
            description:
              "Please drop a file or copy an image to clipboard first.",
            type: "info",
          });
        } catch (error) {
          addToast({
              ...
              {!terminalMinimized && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>Minimize terminal (⌘+J)</TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )}
              {terminalMinimized && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>Maximize terminal (⌘+J)</TooltipContent>
                  </Tooltip>
                </TooltipProvider>
