    ...
    <Dialog open={open} onOpenChange={(next) => (!next ? onClose() : undefined)}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="rounded-md border border-border bg-muted/30 p-3 text-sm">
          <p className="font-medium text-foreground">{conflict?.workspace_name}</p>
          <p className="text-muted-foreground">
            Workspace branch <span className="font-mono">{conflict?.branch_name}</span>
          </p>
        </div>
          ...
          <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                ...
                >
                  <div className="flex items-baseline justify-between gap-4">
                    <div className="font-mono text-sm text-foreground">
                      ...
                      <span className="text-muted-foreground"> ({commit.change_id})</span>
                    </div>
                    <span
                  ...
                  <p className="mt-1 text-sm text-foreground">{commit.description}</p>
            ...
            {commits.length === 0 && (
              <div className="rounded-md border border-dashed border-border p-4 text-center text-sm text-muted-foreground">
                Unable to load conflicting revisions. Try running <code>jj bookmark list</code> manually.
              </div>
            )}
          </div>
        </div>
