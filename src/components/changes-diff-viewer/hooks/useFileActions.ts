...
import { createCommit, getWorkspaceFileLines, jjRestoreAll, jjRestoreFile, jjSplit } from "../../../lib/api";
...
export function useFileActions({
	...
	const handleDiscardFiles = useCallback(async (filePath: string) => {
		...
		try {
			...
			addToast({ description: count === 1 ? `${filesToDiscard[0]} discarded` : `${count} files discarded`, title: "Discarded", type: "success" });
	...
	}, [readOnly, selectedUnstagedFiles, refresh, addToast, invalidateCache, workspacePath, setSelectedUnstagedFiles]);
	...
	}, [workspacePath, addToast, invalidateCache, stagedFiles, setStagedFiles, loadChangedFiles, refreshCommittedChanges, repoPath, workspaceId]);
		...
		try {
			...
			if (result.lines.length > 0) {
				setExpandedContext((prev) => {
					...
					next.set(key, direction === "before" ? [...result.lines, ...prevLines] : [...prevLines, ...result.lines]);
					return next;
				});
			}
		} catch { /* context expansion non-critical */ }
