...
interface UseFileLoadingParams {
	...
	setLargeChangesetExpandedRef: React.MutableRefObject<React.Dispatch<React.SetStateAction<boolean>>>;
	...
	setStaleFilesRef: React.MutableRefObject<React.Dispatch<React.SetStateAction<Set<string>>>>;
	/** Ref updated each render with current setPendingHunksData (breaks circular dep) */
	setPendingHunksDataRef: React.MutableRefObject<React.Dispatch<React.SetStateAction<Map<string, FileHunksData> | null>>>;
...
export function useFileLoading({
	...
	const loadAllFileHunks = useCallback(
			...
			await Promise.all(filesToLoad.map(async (file) => {
				try {
					...
					if (cache?.data) {
						...
						if (hunks) { cachedHunksMap.set(file.path, hunks); hunksMap.set(file.path, { filePath: file.path, hunks, isLoading: false }); }
					}
			...
			try {
				...
				if (isInReviewModeRef.current && !forceApply && !isReloadingRef.current) {
					...
					if (changedFiles.size > 0) {
						setStaleFilesRef.current((prev) => new Set([...prev, ...changedFiles]));
		...
		[workspacePath, isInReviewModeRef, isReloadingRef, setStaleFilesRef, setPendingHunksDataRef, repoPath, workspaceId, allFileHunks],
	);
