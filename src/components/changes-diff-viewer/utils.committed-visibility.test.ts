...
describe("committed Show visibility", () => {
  ...
  it("shows every committed file when Show is on", () => {
    expect(
      shouldShowCommittedFile("clean.txt", true, alwaysVisible),
    ).toBe(true);
    expect(
      shouldShowCommittedFile("dirty.txt", true, alwaysVisible),
    ).toBe(true);
  });

  it("hides only clean committed files when Show is off", () => {
    expect(
      shouldShowCommittedFile("clean.txt", false, alwaysVisible),
    ).toBe(false);
    expect(
      shouldShowCommittedFile("dirty.txt", false, alwaysVisible),
    ).toBe(true);
    expect(
      shouldShowCommittedFile("conflict.txt", false, alwaysVisible),
    ).toBe(true);
  });
