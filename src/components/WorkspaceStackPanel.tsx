...
export const WorkspaceStackPanel = memo<WorkspaceStackPanelProps>(
	({ repoPath, workspace, defaultBranch, onSelectWorkspace }) => {
		...
		// Rendered base-first (closest to the target branch at top, current
		// tip at the bottom), the reverse of the stack's tip-first order.
		...
		const commitQueries = useQueries({
			queries: orderedStack.map((entry) => ({
		...
		const diffStatsByWorkspaceId = useMemo(() => {
			...
			orderedStack.forEach((entry, index) => {
				map.set(
					entry.workspace.id,
