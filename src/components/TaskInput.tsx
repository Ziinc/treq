...
export const TaskInput: React.FC<TaskInputProps> = ({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    searchWorkspaceFiles(repoPath, workspaceId, debouncedMentionQuery, 4).then(
      (results) => {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (
        mentionRef.current && !mentionRef.current.contains(target) &&
  ...
  const handleSubmit = useCallback(
    async (mode: "plan" | "acceptEdits") => {
      ...
      setSubmitting(true);
      try {
        const sessionName = trimmed.length > 50 ? trimmed.slice(0, 47) + "..." : trimmed;
    ...
    [taskText, submitting, repoPath, workspaceId, workspacePath, workingDirectory, onSessionCreated, addToast]
  );
  ...
  const handleKeyDown = useCallback(
    ...
    [handleSubmit, mentionQuery, mentionResults, mentionIndex, handleMentionSelect]
  );
      ...
      <div className="max-w-2xl mx-auto w-full">
        ...
        >
          ...
          <div
            ...
            style={{
              gridTemplateRows: mentionQuery !== null && mentionResults.length > 0 ? "1fr" : "0fr",
            }}
          >
