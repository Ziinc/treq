...
export const ChangesDiffViewer = memo(
	forwardRef<ChangesDiffViewerHandle, ChangesDiffViewerProps>(
		(
			...
			const [conflictMarkerStyle, setConflictMarkerStyle] =
				useState<string>("git");
			...
			const scrollToSearchMatch = useCallback(
				(matchIdx: number) => {
					if (!diffContainerRef.current || searchData.matches.length === 0)
						return;
			...
			useFocusRefreshSubscription("afterInvalidate", () => {
				loadChangedFiles();
			}, [loadChangedFiles]);
			...
			useEffect(() => {
				const loadReview = async () => {
					if (repoPath && workspaceId !== undefined) {
						try {
							const pendingReview = await loadPendingReview(
								repoPath,
								workspaceId,
							);
							if (pendingReview) {
					...
					for (const comment of conflictComments.values()) {
						if (comment.text.trim()) {
							...
							const region = regions?.find(
								(r) => r.conflictNumber === comment.conflictNumber,
							);
								...
								<>
									...
									{hasRoomAbove && (
										<div
											...
											onClick={() =>
												handleExpandContext(filePath, hunkIndex, hunk, "before")
											}
										>
											<div className="w-24 flex-shrink-0 border-r border-border/40" />
											<div className="w-6 flex-shrink-0" />
							...
							if (newLineNumber !== undefined && conflictLineMap) {
								...
								if (conflictRegion) {
									if (
										...
										conflictRegion.startLine === newLineNumber
									) {
							...
							return (
								<>
									<div
										className="flex items-stretch font-mono text-sm bg-muted/30 hover:bg-muted/50 cursor-pointer transition-colors"
										onClick={() =>
											handleExpandContext(filePath, hunkIndex, hunk, "after")
										}
									>
										<div className="w-24 flex-shrink-0 border-r border-border/40" />
										<div className="w-6 flex-shrink-0" />
									...
									{afterLines &&
										...
										afterLines.map((ctxLine, ctxIdx) => {
											const { newStart, newCount } = parseHunkHeader(
												hunk.header,
											);
			...
			const handleSelectAllStaged = useCallback(() => {
				setSelectedStagedFiles((prev) => {
					// Toggle: if all selected, deselect all
					if (
						prev.size === stagedFilesList.length &&
					...
					if (selectedStagedFiles.size > 0) {
						...
						if (unstagedFiles.length > 0) {
							setLastSelectedFileIndex(
								files.findIndex(
									(f) =>
										f.path === unstagedFiles[unstagedFiles.length - 1].path,
								),
							);
						}
					}
								...
								<>
									...
									{actualConflictedFiles.length > 0 && (
										<ConflictsSection
											...
											onToggleCollapse={() =>
												toggleSectionCollapse("conflicts")
											}
					...
					<div className="flex-1 flex flex-col min-w-0">
													...
													<div className="flex justify-between gap-2">
														...
														<div className="flex gap-2">
															...
															<Button
																size="sm"
																onClick={() =>
																	handleRequestChanges("acceptEdits")
																}
																disabled={sendingReview}
															>
																Edit
						...
						<div className="flex-1 overflow-hidden relative">
							<SearchOverlay
								...
								currentMatch={
									searchData.matches.length > 0 ? currentMatchIndex + 1 : 0
								}
												...
												{actualConflictedFiles.length > 0 && (
													<>
														{actualConflictedFiles.map((conflictFile) => {
															const regions =
																conflictRegionsByFile.get(conflictFile);
