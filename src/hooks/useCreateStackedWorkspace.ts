    ...
    async ({
      ...
      try {
        ...
        const branchPattern = userBranchName
          ...
          : (await getRepoSetting(repoPath, "branch_name_pattern").catch(() => null)) || "treq/{name}";
        ...
        if (userBranchName) {
          ...
          let index = 1;
          do {
            branchName = generateStackedBranchName(branchPattern, parentBranch, index);
            index++;
        ...
        const effectiveParentBranch = position === "before" && parentWorkspace?.target_branch
