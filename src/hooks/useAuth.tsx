...
import { supabase, SUPABASE_URL, SUPABASE_ANON_KEY, WEB_URL } from "../lib/supabase";
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const persistSession = useCallback(async (sess: Session | null) => {
    if (sess) {
      await setSetting("supabase_session", JSON.stringify({
  ...
  const value = useMemo(
    () => ({ user, session, loading, subscription, signIn, signOut, refreshSubscription, exchangeToken }),
    [user, session, loading, subscription, signIn, signOut, refreshSubscription, exchangeToken]
  );
