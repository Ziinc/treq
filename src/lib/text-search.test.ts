...
describe("text-search utilities", () => {
  ...
  describe("highlightInHtml", () => {
    ...
    it("wraps matches while preserving Prism.js spans", () => {
      const html =
        '<span class="token keyword">function</span> <span class="token function">test</span>';
    ...
    it("highlights text content while preserving surrounding HTML tags", () => {
      const html =
        '<span class="token keyword">const</span> x = <span class="token number">42</span>';
      const result = highlightInHtml(html, "const", -1);

      // Should highlight "const" in text content only
      expect(result.html).toBe(
        '<span class="token keyword"><mark class="search-match">const</mark></span> x = <span class="token number">42</span>',
      );
      expect(result.matchCount).toBe(1);
    });
    ...
    it("does not match text that spans across HTML tag boundaries", () => {
      const html =
        '<span class="token">hel</span><span class="token">lo</span>';
    ...
    it("correctly counts matches in HTML content", () => {
      const html =
        '<span class="token">test</span> another <span>test</span> and test';
      const result = highlightInHtml(html, "test", -1);
    ...
    it("highlights only the specified current match", () => {
      ...
      const currentMatchCount = (
        result.html.match(/search-match-current/g) || []
      ).length;
      const regularMatchCount = (result.html.match(/search-match"/g) || [])
        .length;
