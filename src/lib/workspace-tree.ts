...
export interface WorkspaceTargetMoveStep {
	workspace: Workspace;
	newTargetBranch: string;
}

/**
 * Plan target_branch updates to make `movingBranch` target `newTargetBranch`
 * without creating a cycle.
 *
 * When the new target is a descendant (parent dragged below a child), the
 * immediate child of `movingBranch` on the path to the new target is lifted
 * onto `movingBranch`'s current parent first, then the move is applied.
 */
export function planWorkspaceTargetMove(
	workspaces: Workspace[],
	movingBranch: string,
	newTargetBranch: string,
	defaultBranch = "main",
	...
	const moving = workspaceByBranch.get(movingBranch);
	if (!moving) {
		return [];
	}

	if (movingBranch === newTargetBranch) {
		throw new Error("Cannot target self: would create a cycle");
	}

	const steps: WorkspaceTargetMoveStep[] = [];

	if (isDescendantOf(workspaces, newTargetBranch, movingBranch)) {
		// Walk from the new target up to find the bridge child of movingBranch.
		let current = newTargetBranch;
		const visited = new Set<string>();
		let bridge: Workspace | undefined;
		...
		while (true) {
			...
			if (ws.target_branch === movingBranch) {
				bridge = ws;
				break;
			}
			current = ws.target_branch;
		...
		if (bridge) {
			const liftTarget = moving.target_branch ?? defaultBranch;
			if (liftTarget !== bridge.branch_name) {
				steps.push({
					workspace: bridge,
					newTargetBranch: liftTarget,
				});
			}
		}
	}
