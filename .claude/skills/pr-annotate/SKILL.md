...
1. **No attribution footer.** Do not append `_Generated by [Claude
   Code](...)_` or any `---` divider to review comments. That footer is for
   PR bodies and top-level replies (per the repo's GitHub-posting
   convention) — not for per-file inline annotations, which are Claude's
   working notes on the diff, not a standalone authored post.
