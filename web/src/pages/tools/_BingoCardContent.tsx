...
function buildBoard(topic: Topic): string[] {
...
function checkBingo(marked: Set<number>): number[][] {
  ...
  for (let r = 0; r < GRID_SIZE; r++) {
    ...
    if (row.every((i) => marked.has(i))) wins.push(row);
  }

  // Columns
  for (let c = 0; c < GRID_SIZE; c++) {
    const col = Array.from({ length: GRID_SIZE }, (_, r) => r * GRID_SIZE + c);
    if (col.every((i) => marked.has(i))) wins.push(col);
  }
  ...
  if (diag1.every((i) => marked.has(i))) wins.push(diag1);
  if (diag2.every((i) => marked.has(i))) wins.push(diag2);
...
export function BingoCardContent() {
  ...
  const [board, setBoard] = useState<string[]>(() => buildBoard(TOPICS[0]));
  const [marked, setMarked] = useState<Set<number>>(() => new Set([FREE_INDEX]));
  const [winLines, setWinLines] = useState<Set<number>>(new Set());
  ...
  const randomise = useCallback(() => {
    ...
    const fresh = new Set<number>([FREE_INDEX]);
    setMarked(fresh);
    setWinLines(new Set());
    setJustWon(false);
  }, [activeTopicId]);
      ...
      <div className={styles.content}>
        <div className={styles.tabs}>
          {TOPICS.map((topic) => (
            <button
              key={topic.id}
              className={`${styles.tab} ${topic.id === activeTopicId ? styles.tabActive : ''}`}
        ...
        <div className={styles.controls}>
          ...
          {justWon && (
            <span className={styles.bingoBadge}>🎉 BINGO!</span>
          )}
        </div>
        ...
        <div className={styles.cardWrapper}>
          <div className={styles.headerRow}>
            {['B', 'I', 'N', 'G', 'O'].map((letter) => (
              <div key={letter} className={styles.headerCell}>
                {letter}
              </div>
            ))}
          </div>
