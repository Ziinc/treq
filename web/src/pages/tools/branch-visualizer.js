...
function renderGraph(svgEl, state, dark) {
    // Clear
    ...
    for (const n of nodes) {
        if (!branchCols.has(n.commit.branch))
            branchCols.set(n.commit.branch, n.x);
    }
...
function BranchEditor({ branches, onChange, }) {
    ...
    return (<div className={styles.section}>
      ...
      {branches.map((b, i) => (<div key={i} className={styles.row}>
          <input className={styles.input} value={b.name} onChange={(e) => update(i, 'name', e.target.value)} placeholder="branch name"/>
          <input type="color" className={styles.colorPicker} value={b.color} onChange={(e) => update(i, 'color', e.target.value)}/>
          <button className={styles.removeBtn} onClick={() => remove(i)}>✕</button>
        </div>))}
    </div>);
}
function CommitEditor({ commits, branches, onChange, }) {
    const add = () => onChange([
        ...commits,
        { id: `c${commits.length + 1}`, label: 'New commit', branch: branches[0]?.name ?? 'main', parents: [] },
    ]);
    const remove = (i) => onChange(commits.filter((_, idx) => idx !== i));
    const update = (i, field, val) => onChange(commits.map((c, idx) => (idx === i ? { ...c, [field]: val } : c)));
    const toggleParent = (i, parentId) => {
    ...
    return (<div className={styles.section}>
      ...
      {commits.map((c, i) => (<div key={i} className={styles.commitBlock}>
          <div className={styles.row}>
            <input className={styles.input} style={{ width: '80px' }} value={c.id} onChange={(e) => update(i, 'id', e.target.value)} placeholder="id"/>
            <input className={styles.input} value={c.label} onChange={(e) => update(i, 'label', e.target.value)} placeholder="label"/>
            <select className={styles.select} value={c.branch} onChange={(e) => update(i, 'branch', e.target.value)}>
          ...
          <div className={styles.parentRow}>
            ...
            {commits.filter((_, pi) => pi < i).map((p) => (<label key={p.id} className={styles.parentCheck}>
                <input type="checkbox" checked={c.parents.includes(p.id)} onChange={() => toggleParent(i, p.id)}/>
                {p.id}
              </label>))}
          </div>
        </div>))}
...
export default function BranchVisualizerPage() {
    ...
    const copyPng = () => {
        ...
        img.onload = () => {
            ...
            canvas.toBlob((pngBlob) => {
                if (!pngBlob)
                    return;
                navigator.clipboard.write([new ClipboardItem({ 'image/png': pngBlob })]).then(() => showToast('PNG copied to clipboard!'));
            }, 'image/png');
    ...
    // ── Render ──────────────────────────────────────────────────────────────────
    return (<Layout title="Branch Visualizer" description="Visualize git branch structures with a hand-drawn aesthetic. Shareable via URL.">
        ...
        <div className={styles.workspace}>
          ...
          <div className={styles.editor}>
            <div className={styles.section}>
              ...
              <input className={styles.input} style={{ marginTop: '6px' }} value={graphState.title} onChange={(e) => setGraphState((s) => ({ ...s, title: e.target.value }))}/>
            </div>

            <BranchEditor branches={graphState.branches} onChange={(branches) => setGraphState((s) => ({ ...s, branches }))}/>

            <CommitEditor commits={graphState.commits} branches={graphState.branches} onChange={(commits) => setGraphState((s) => ({ ...s, commits }))}/>
          </div>
