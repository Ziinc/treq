...
import { ReactFlow, ReactFlowProvider, Background, BackgroundVariant, Controls, MiniMap, Panel, Handle, Position, addEdge, useNodesState, useEdgesState, useReactFlow, MarkerType, } from '@xyflow/react';
...
function stateToEdges(s) {
    return s.edges.map((e) => ({ ...e, ...EDGE_DEFAULTS }));
}
// ── Custom WorkflowNode ───────────────────────────────────────────────────────
function WorkflowNode({ data, selected }) {
    return (<div className={`${styles.node} ${selected ? styles.nodeSelected : ''}`} data-testid="flow-node">
      ...
      {data.prompt && (<div className={styles.nodePrompt}>
          {data.prompt.length > 80 ? data.prompt.slice(0, 77) + '…' : data.prompt}
        </div>)}

      {data.skills.length > 0 && (<div className={styles.nodeSkills}>
          {data.skills.map((s) => (<span key={s} className={styles.skill}>{s}</span>))}
        </div>)}
...
function SkillInput({ onAdd }) {
    ...
    const filtered = val
        ? SKILL_SUGGESTIONS.filter((s) => s.includes(val.toLowerCase()) || val.toLowerCase().includes(s.slice(1, 4)))
        : SKILL_SUGGESTIONS;
    const commit = (s) => {
    ...
    return (<div className={styles.skillInputWrap} ref={ref}>
      <div className={styles.skillInputRow}>
        <input className={styles.skillInput} value={val} placeholder="/skill-name" onChange={(e) => { setVal(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)} onKeyDown={onKey}/>
        <button className={styles.addSkillBtn} onClick={() => commit(val)}>Add</button>
      </div>
      {open && filtered.length > 0 && (<div className={styles.skillDropdown}>
          {filtered.map((s) => (<div key={s} className={styles.skillOption} onMouseDown={() => commit(s)}>
              {s}
            </div>))}
        </div>)}
    </div>);
}
function NodeDetailPanel({ node, onChange, onDelete, onClose }) {
    const { data, id } = node;
    return (<div className={styles.detailPanel}>
      ...
      <div className={styles.panelBody}>
        ...
        <input className={styles.fieldInput} value={data.name} onChange={(e) => onChange(id, { name: e.target.value })}/>

        <label className={styles.fieldLabel}>Prompt</label>
        <textarea className={styles.fieldTextarea} rows={6} value={data.prompt} onChange={(e) => onChange(id, { prompt: e.target.value })} placeholder="Describe what the AI agent should do at this step…"/>

        <label className={styles.fieldLabel}>Slash Skills</label>
        <div className={styles.skillTags}>
          {data.skills.map((s) => (<span key={s} className={styles.skillTag}>
              {s}
              <button className={styles.skillRemove} onClick={() => onChange(id, { skills: data.skills.filter((x) => x !== s) })} aria-label={`Remove ${s}`}>
                ✕
              </button>
            </span>))}
        </div>
        <SkillInput onAdd={(skill) => {
            if (!data.skills.includes(skill))
                onChange(id, { skills: [...data.skills, skill] });
        }}/>
      </div>
...
function DagCanvas() {
    ...
    // Connect handler
    const onConnect = useCallback((conn) => setEdges((eds) => addEdge({ ...conn, ...EDGE_DEFAULTS, id: `e${conn.source}-${conn.target}` }, eds)), [setEdges]);
    // Add node
    ...
    return (<div className={`${styles.workspace} ${selectedNode ? styles.hasPanel : ''}`}>
      {toast && <div className={styles.toast}>{toast}</div>}

      <div className={styles.flowContainer}>
        <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeClick={onNodeClick} onPaneClick={onPaneClick} nodeTypes={nodeTypes} fitView fitViewOptions={{ padding: 0.12 }} deleteKeyCode="Delete" proOptions={{ hideAttribution: true }}>
      ...
      {selectedNode && (<NodeDetailPanel node={selectedNode} onChange={updateNode} onDelete={deleteNode} onClose={() => setSelectedId(null)}/>)}
    </div>);
}
