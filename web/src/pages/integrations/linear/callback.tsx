  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!session) {
        ...
        setMessage(
          "Please sign in, then restart the Linear connection from the dashboard."
        );
        return;
      }
      ...
      setState("success");
      setMessage(data?.workspace_name ?? "Linear");
    ...
    <div style={styles.container}>
      {state === "loading" && (
        <>
          ...
          <p style={styles.text}>Connecting Linear…</p>
        </>
      )}
      ...
      {state === "success" && (
        <>
          ...
          <p style={styles.text}>
            Connected to Linear workspace: {message}. Redirecting to your dashboard…
          </p>
        </>
      )}
...
export default function LinearCallbackPage(): React.ReactNode {
  ...
  return (
    <Layout title="Linear Integration" description="Connecting your Linear workspace">
