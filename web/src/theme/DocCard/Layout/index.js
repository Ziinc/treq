...
function Container({ className, href, children, }) {
    return (<Link href={href} className={clsx('card', ThemeClassNames.docs.docCard.container, styles.cardContainer, className)}>
      {children}
    </Link>);
}
export default function DocCardLayout({ item, className, href, icon, title, description, }) {
    return (<Container href={href} className={className}>
