...
function IconChatGPT() {
    return (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ flexShrink: 0 }}>
...
function IconMarkdown() {
    return (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 208 128" fill="currentColor" aria-hidden="true" style={{ marginRight: '0.3rem', verticalAlign: 'middle', flexShrink: 0 }}>
 ...
 */
function resolveLinks(markdown, permalink) {
    const base = new URL(permalink, window.location.origin).href;
    return markdown.replace(/(!?\[[^\]]*\])\(([^)]+)\)/g, (match, bracketPart, urlAndTitle) => {
...
export default function EditMetaRow({ className, editUrl, lastUpdatedAt, lastUpdatedBy, }) {
    ...
    const markdownData = usePluginData('raw-markdown-plugin');
    ...
    const aiPrompt = encodeURIComponent(`Read from ${pageUrl} so I can ask questions about its contents`);
    const chatGptUrl = `https://chatgpt.com/?hint=search&q=${aiPrompt}`;
    const claudeUrl = `https://claude.ai/new?q=${aiPrompt}`;
    return (<div className={clsx('row', className)}>
      ...
      <div className={clsx('col', styles.lastUpdated)}>
        {(lastUpdatedAt || lastUpdatedBy) && (<LastUpdated lastUpdatedAt={lastUpdatedAt} lastUpdatedBy={lastUpdatedBy}/>)}
      </div>
    </div>);
}
