...
Treq writes a `treq` skill into a temporary directory when it starts the agent. The skill is not copied into the repository. You do not install it.
...
Claude Code loads the skill through `--add-dir`. Cursor Agent loads it through `--plugin-dir`. Codex receives the skill file path in its developer instructions.
