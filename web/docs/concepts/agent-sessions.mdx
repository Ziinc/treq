...
Claude Code loads the skill through `--add-dir`. Cursor Agent loads it through `--plugin-dir`. Codex also receives a copy at `.agents/skills/treq/` in the working directory, which is the path Codex scans for skills. Treq's snapshot ignore list excludes that folder, so it does not show up as a workspace change. The copy is deleted when the agent process exits.
