...
Claude Code loads `.claude/skills/treq/`. Codex and Cursor Agent load `.agents/skills/treq/`. Treq's snapshot ignore list excludes those folders, so they do not show up as workspace changes or land in commits. The copies are deleted when the agent process exits.
