...
Claude Code loads `.claude/skills/treq/`. Codex and Cursor Agent load `.agents/skills/treq/`. Treq appends `.agents/skills/treq*/` and `.claude/skills/treq*/` to `.gitignore`. The same globs are in Treq's snapshot ignore list, so `treq commit` and external `git` both skip those folders. The copies are deleted when the agent process exits.
