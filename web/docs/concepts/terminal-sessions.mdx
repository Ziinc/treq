...
Three-finger swipe up opens Terminal Mission Control: every open terminal as a card, grouped by workspace and sorted by recent activity, in a two-column grid. Each card shows a preview of recent terminal output, with a small status icon for streaming, active, or idle. Select a card to focus that terminal. Three-finger swipe down or `Escape` closes the overlay.
