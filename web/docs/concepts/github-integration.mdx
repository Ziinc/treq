...
Pull Requests and Issues use a dual-pane list and detail layout. Issue filters are Open, Closed, and All. Pull Request filters add Draft to the left of Open, and Draft lists only draft PRs. From a PR you can read the body and conversation, add a comment, mark draft or ready, close or reopen, and open the PR on GitHub. The list also has a New form for creating a PR by hand with title, body, base, and head.
