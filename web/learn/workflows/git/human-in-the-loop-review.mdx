...
Passing tests do not prove that the task was implemented correctly. The agent can encode the same false assumption in code and tests, which is why an [AI review pass](/learn/ai-code-review) sits before human approval, not in place of it.
...
Track review state with the task, branch, owner, risk level, and current checkpoint. Reviewing a [stack of dependent changes](/learn/concepts/git/stacked-prs) means tracking each layer separately. Assign security-sensitive or architectural changes to reviewers who know that area.
