...
This article teaches the Git worktree pattern. [Treq](/docs/concepts/workspaces) uses managed workspaces under `.treq/workspaces/` instead of `git worktree add`. Treq handles repository operations in the app and auto-rebases stacks when targets move. You do not need Jujutsu knowledge for that workflow.
