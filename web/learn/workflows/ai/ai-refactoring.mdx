...
Prefer a codemod or a language-server refactor when it can express the change exactly. Use an agent when the change needs context from the whole repository, or when call sites need different edits.
...
Run refactors that touch shared interfaces one after another. Parallel agents editing the same callers create conflicts and make the diff harder to review.
