...
The agent works in an **isolated workspace** with its own files and branch. That keeps your other work safe from its edits and commands. It does not make the output safe to merge.
...
Give each agent its own workspace and branch. Say who owns what when tasks touch shared interfaces. Run dependent tasks in order, or settle the interface before they start in parallel.
