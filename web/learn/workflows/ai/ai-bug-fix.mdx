...
Tests are evidence, not proof of the complete diagnosis. A test can encode the same misunderstanding as the patch, which is why the [human-in-the-loop review](/learn/ai-code-review) still has to read the diagnosis. Review whether its setup matches the real failure and whether its assertions distinguish the correct behavior from a convenient workaround.
