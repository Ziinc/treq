...
Jujutsu logs repository-changing commands in an [operation log](./version-control). `jj undo` and `jj op log` cover a similar recovery need with a different command model.

In Treq, [discard and abandon](/docs/how-to/discarding-changes) restore from the **Undo** button on the success toast while that operation is still HEAD. Use the reflog for Git-side ref movement, not for those Treq working-copy actions.
...
- [Recover a Deleted Git Branch](/learn/how-to/recover-deleted-git-branch): recreate a deleted branch from a `HEAD` reflog tip
