...
A **repository** contains versioned files and the history needed to reconstruct their recorded states. In a distributed system such as Git, each normal [clone](./git-worktrees-vs-clones) contains repository history and can create commits without contacting a central server.
...
A **commit** records a project snapshot plus metadata and parent commit IDs. Those parent links form a directed acyclic graph. A commit with one parent continues a line of history, while a [merge commit](./merge-vs-rebase) has multiple parents.
...
Use `git log` to inspect ancestry, `git show` to inspect one commit, and `git blame` to trace lines to commits. Use `git revert` to undo a published change with a new commit. [Discarding uncommitted changes](/learn/how-to/discarding-changes) is a different operation and cannot be undone. History-rewriting commands such as rebase and reset require more care because other work may depend on the old commit IDs.
