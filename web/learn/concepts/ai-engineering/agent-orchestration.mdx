...
These patterns can be combined. A supervisor might divide a migration into independent tasks, run workers in parallel, and then pass their changes to a review agent. The important part is that the dependencies are explicit. If two tasks both depend on an unfinished change, running them at the same time creates rework and saves nothing.

Each agent also needs an isolated workspace. Two agents editing the same directory can overwrite each other's changes or leave the working tree in an unexpected state. Give every agent its own [Git worktree](/learn/concepts/git/git-worktrees), branch, sandbox, or clone, then review and merge each result deliberately.
