...
Graphite and Treq operate at different parts of the development workflow, but AI teams often evaluate them together. Graphite is a stacked pull request and code review platform. Treq is a local Stacking ADE that manages dependent agent workspaces before you open a pull request. The landing page calls Treq an open-source Graphite alternative for that reason. The longer comparison of workflow philosophy lives in [Treq as a Graphite alternative](/learn/graphite-alternative).

The comparison matters because agent throughput changes where the pain shows up. When people wrote most of the code, stack tools focused on review batching and merge order. When agents write most of the code, you also need isolation. You need comment routing back into agent sessions. You need ongoing rebase hygiene. You need conflict work to go back to the agent.
...
**Treq** creates isolated workspaces for coding agents, shows diffs for review, and turns comments into the next agent prompt. Workspaces target other workspaces to form a stack before any pull request exists. When a lower layer moves, Treq rebases dependents and shows conflicts. That is the landing-page loop: stacked workspaces, auto-rebase, auto-resolve.

**Graphite** helps teams create, review, restack, and merge stacked pull requests. It sits on top of GitHub's pull request model and optimizes the social and process layer of code review: smaller diffs, clearer dependencies, and better merge sequencing. It is not a desktop environment for launching Claude, Codex, or Cursor agents in parallel workspaces.
...
| GitHub PR collaboration | After push, through normal Git tooling. App integration is [coming soon](/roadmap) | Native product focus |
| Merge queue | Treq-managed queue planned, not GitHub Enterprise Cloud | Graphite merge workflows on GitHub |
...
Treq's [roadmap](/roadmap) adds GitHub pull request status and a Treq-managed merge queue so a local stack can land on `main` without GitHub Enterprise Cloud. That is still planned work. Graphite already owns the hosted PR and merge process. Until those Treq features ship, Graphite remains the stronger GitHub collaboration product.

If your team already has a strong Graphite workflow, Treq can still help on the generation side. Agents work in isolated stacks locally, you review there, then you push into whatever PR process you already trust. If your team has no agent orchestration layer yet, starting with Graphite alone leaves the parallel generation problem unsolved. Agents still collide in one checkout. See [parallel coding agents](/learn/parallel-coding-agents).

## Pricing and Licensing

Treq's desktop app is free and open source under Apache 2.0. [Pricing](/pricing) lists Free and Pro at $15 per user per month. Pro is for GitHub on private repos and merge queue. Those features are marked coming soon. Local review stays on your machine either way.
...
- Need review comments and conflict instructions to go back into the agent loop.
...
- Need hosted collaboration features around PR review and merge queues today.
...
- [Treq as a Graphite alternative](/learn/graphite-alternative): workflow philosophy, not a feature checklist.
