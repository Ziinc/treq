...
/// content AND `jj status` must report a clean working copy. The current
/// implementation rewrites the working-copy commit but never resets the
/// on-disk working copy, leaving wc commit and disk diverged — which panics
/// the next jj-lib call and tears down the app.
#[test]
