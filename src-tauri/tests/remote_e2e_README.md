...
| 5 | Client authenticates with user-selected key + short-lived cert; Treq never generates a private key | `remote_e2e.test.ts` "issues a short-lived certificate...", "issued certificates carry a bounded, short expiry"; `remote_ssh_server_it.rs` certificate auth against real `sshd`; `RemoteSetupDialog.test.tsx` "shows region and size pickers and the key fingerprint..."; `remote_local_keys.rs::returns_empty_when_ssh_dir_missing` | deno-e2e, unit, server-it | Supabase test project; jsdom; CI `sshd` | **skipped** (e2e / server-it without creds); **pass** (UI unit) | Control plane issues bounded cert; UI shows fingerprint before registration; production transport authenticates with `ssh-keygen -s` user certs against a trusted CA on live `sshd`. **Gap:** Deno signer vs `ssh-keygen` format parity is assumed, not executed in the same process. |
...
| 17 | Shell and agent PTYs start in the selected remote workspace | `remote_ssh_transport.rs::pty_open_and_close_record_start_and_exit_counts`, `pty_open_in_directory_execs_a_quoted_cd_before_the_command`; `remote.rs` `builds_ssh_shell_command_with_working_dir`, `quotes_remote_paths_with_single_quotes`; `remote_ssh_server_it.rs` PTY cwd/IO/resize/close | unit, server-it | Mock `russh`; real `sshd` | **partial** (units); **skipped** (server-it) | PTY open/close, quoted `cd` into the selected directory, and live `sshd` PTY cwd/IO/resize/close. **Gap:** no coding-agent PTY on a provisioned Treq VM. |
...
- **OpenSSH KRL / `RevokedKeys` (#7)** — live `sshd` rejects expired/invalid certs; client-key revocation is `force_cutoff`, not a server KRL.
- **Deno Edge Function signer vs `ssh-keygen -s` in one process (#5)** — both emit OpenSSH user certs; the CI job uses `ssh-keygen -s`.
