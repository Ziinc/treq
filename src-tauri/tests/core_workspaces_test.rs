...
#[test]
fn test_home_workspace_status_populates_conflicted_files() {
    let repo = TestRepo::new().expect("Failed to create test repo");

    repo.create_file("home-conflict.txt", "side-a\n")
        .expect("write side a");
    treq_lib::jj::jj_commit(&repo.repo_path, "side a").expect("commit side a");
    let side_a = TestRepo::run_jj(
        &repo.repo_path,
        &["log", "-r", "@-", "--no-graph", "-T", "change_id\n"],
    )
    .expect("side a change id")
    .trim()
    .to_string();

    let parent = TestRepo::run_jj(
        &repo.repo_path,
        &["log", "-r", &format!("{side_a}-"), "--no-graph", "-T", "change_id\n"],
    )
    .expect("parent change id")
    .trim()
    .to_string();

    TestRepo::run_jj(&repo.repo_path, &["new", &parent]).expect("new on parent");
    repo.create_file("home-conflict.txt", "side-b\n")
        .expect("write side b");
    treq_lib::jj::jj_commit(&repo.repo_path, "side b").expect("commit side b");
    let side_b = TestRepo::run_jj(
        &repo.repo_path,
        &["log", "-r", "@-", "--no-graph", "-T", "change_id\n"],
    )
    .expect("side b change id")
    .trim()
    .to_string();

    TestRepo::run_jj(&repo.repo_path, &["new", &side_a, &side_b]).expect("merge conflicting sides");

    let direct = treq_lib::jj::get_conflicted_files(&repo.repo_path, None).unwrap_or_default();
    let has_unresolved =
        treq_lib::jj::workspace_has_unresolved_conflicts(&repo.repo_path).unwrap_or(false);
    println!("direct conflicted_files={direct:?} has_unresolved={has_unresolved}");

    let status = treq_lib::core::workspace_status(&repo.repo_path, None)
        .expect("home workspace_status should succeed");
    println!(
        "status has_conflicts={} conflicted_files={:?}",
        status.partial.has_conflicts, status.conflicted_files
    );

    assert!(
        status.partial.has_conflicts || !status.conflicted_files.is_empty() || !direct.is_empty(),
        "home has_conflicts should be true; direct={direct:?} unresolved={has_unresolved} status={:?}",
        status.conflicted_files
    );
    assert!(
        status
            .conflicted_files
            .iter()
            .any(|f| f == "home-conflict.txt"),
        "ShowWorkspace reads conflicted_files for the alert; got {:?}",
        status.conflicted_files
    );
}
