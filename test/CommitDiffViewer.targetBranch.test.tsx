...
describe("CommitDiffViewer - Target Branch History", () => {
  ...
  it("target branch commits have no action buttons", async () => {
    ...
    await waitFor(() => {
      ...
      const expandedSection = document.querySelector(`[data-commit-id="target001"]`);
      ...
      const moveCommitButton = Array.from(moveButtons).find(b => b.textContent?.includes("Move commit"));
      const deleteCommitButton = Array.from(moveButtons).find(b => b.textContent?.includes("Delete commit"));
