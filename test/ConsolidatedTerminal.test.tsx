...
describe("ConsolidatedTerminal autoCommand behavior", () => {
	...
	it("sends autoCommand when creating a NEW PTY session", async () => {
		...
		const mockPtySessionExists = vi
			.spyOn(api, "ptySessionExists")
			.mockResolvedValue(false);
		const mockPtyCreateSession = vi
			.spyOn(api, "ptyCreateSession")
			.mockResolvedValue(undefined);
		const mockPtyListen = vi
			.spyOn(api, "ptyListen")
			.mockResolvedValue(() => {});
		...
		await waitFor(() => {
			expect(mockPtyListen).toHaveBeenCalledWith(
				"test-session-1",
				expect.any(Function),
			);
		});
	...
	it("does NOT send autoCommand when component remounts with existing session", async () => {
		...
		const _mockPtySessionExists = vi
			.spyOn(api, "ptySessionExists")
			.mockResolvedValue(true);
		const mockPtyListen = vi
			.spyOn(api, "ptyListen")
