...
import { findSidebarBranchElement } from "../Dashboard.test";
...
describe("ShowWorkspace - header", () => {
  ...
  it("can change workspace A target branch to workspace B (stacked)", async () => {
    ...
    await waitFor(() => {
      targetBtn = screen.getByRole("button", { name: "Workspace target" }) as HTMLButtonElement;
      expect(targetBtn).not.toBeDisabled();
    });
    fireEvent.click(targetBtn!);

    const betaElement = await screen.findByText("feat/beta", { selector: ".branch-list-item *" });
    ...
    // TargetBranchSelector trigger should show feat/beta as the selected value
     ...
     expect(screen.queryByText("main", {selector: "button *"})).not.toBeInTheDocument();
