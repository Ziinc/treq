...
async function startEditingComment(comment: string) {
  ...
  const commentCard = commentText.closest("[data-testid='inline-comment-card']");
...
describe("Inline comment editing", () => {
  ...
  it("enters edit mode when clicking an inline comment", async () => {
    await setupEditableComment("feat/comment-edit-open", "Original comment text");

    const textarea = await startEditingComment("Original comment text");
    const editForm = textarea.parentElement;
    expect(editForm).toBeTruthy();

    expect(within(editForm!).getByRole("button", { name: /^save$/i })).toBeTruthy();
    expect(
  ...
  it("updates inline comment text when saving", async () => {
    await setupEditableComment("feat/comment-edit-save", "Original comment text");
  ...
  it("discards the inline comment from edit mode", async () => {
    ...
    await user.click(within(editForm!).getByRole("button", { name: /^discard$/i }));
