...
function makeWorkspace(overrides: Partial<Workspace> & { id: number; branch_name: string }): Workspace {
  return {
    repo_path: '/test/repo',
...
describe('useWorkspaceHierarchy', () => {
  ...
  const workspaceA = makeWorkspace({ id: 1, branch_name: 'A', target_branch: 'main' });
  const workspaceB = makeWorkspace({ id: 2, branch_name: 'B', target_branch: 'A' });
  const workspaceC = makeWorkspace({ id: 3, branch_name: 'C', target_branch: 'A' });
  const workspaceD = makeWorkspace({ id: 4, branch_name: 'D', target_branch: 'B' });
  ...
  describe('addBefore', () => {
    test('creates workspace stacked on original target, then reparents original', async () => {
      const { setWorkspaceTargetBranch, getWorkspaces } = await import('../src/lib/api');

      const newWorkspace = makeWorkspace({ id: 10, branch_name: 'new-ws', target_branch: 'A' });
      mockCreateStackedWorkspace.mockResolvedValue(10);
      (getWorkspaces as any).mockResolvedValue([...allWorkspaces, newWorkspace]);
    ...
    test('handles root workspace (target_branch is null) by using "main" as parent', async () => {
      ...
      const rootWorkspace = makeWorkspace({ id: 5, branch_name: 'root-ws', target_branch: null });
      const newWorkspace = makeWorkspace({ id: 10, branch_name: 'new-ws', target_branch: null });
      mockCreateStackedWorkspace.mockResolvedValue(10);
