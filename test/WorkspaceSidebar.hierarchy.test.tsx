...
describe('WorkspaceSidebar hierarchy features', () => {
  const workspaceA = makeWorkspace({ id: 1, branch_name: 'workspace-A', target_branch: 'main' });
  const workspaceB = makeWorkspace({ id: 2, branch_name: 'workspace-B', target_branch: 'workspace-A' });
  const workspaceC = makeWorkspace({ id: 3, branch_name: 'workspace-C', target_branch: 'workspace-A' });
  ...
  describe('action buttons', () => {
    test('renders add-before and add-after buttons for each workspace row', async () => {
      ...
      const addBeforeButtons = screen.getAllByLabelText('Insert workspace before');
      const addAfterButtons = screen.getAllByLabelText('Insert workspace after');
  ...
  describe('double-click selection', () => {
    test('double-click on workspace calls onSelectStack with entire stack', async () => {
      ...
      render(<WorkspaceSidebar {...defaultProps} onSelectStack={onSelectStack} />);
      ...
      expect(onSelectStack).toHaveBeenCalledWith(
        new Set([1, 2, 3])
      );
    });
